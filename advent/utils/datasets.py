...
class RealDataset(Dataset):
    def __init__(self, opts, transform=None, no_check=False):
        ...
        if real_file_list_path.suffix == ".json":
            self.real_samples_paths = self.json_load(real_file_list_path)
        elif real_file_list_path.suffix in {".yaml", ".yml"}:
            self.real_samples_paths = self.yaml_load(real_file_list_path)
        else:
            raise ValueError("Unknown file list type in {}".format(real_file_list_path))
    ...
    def __getitem__(self, i):
        ...
        Returns:
            dict: dataset item where tensors of data are in item["data"] which is a dict
                  {task: tensor}
        """
        paths = self.real_samples_paths[i]
        if self.transform:
            return Dict(
                {
                    "data": self.transform(
    ...
    def __len__(self):
        return len(self.real_samples_paths)

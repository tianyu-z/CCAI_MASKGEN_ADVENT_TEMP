...
def load_opts(path=None, default=None):
    ...
    if default is None:
        ...
        with open(default, "r") as f:
            ...
            default_opts = Dict(yaml.load(f.read()))
    with open(path, "r") as f:
        #overriding_opts = Dict(yaml.safe_load(f))
        overriding_opts = Dict(yaml.load(f.read()))
...
#     """Flattens a multi-level EasyDict or native dictionnary into a single
...
#     }
#     Args:
#         opts (EasyDict or dict): addict dictionnary to flatten
#     Returns:
...
#     def p(d, prefix="", vals=[]):
#         for k, v in d.items():
#             if isinstance(v, (EasyDict, dict)):
#                 p(v, prefix + k + ".", vals)
#             elif isinstance(v, list):
#                 if v and isinstance(v[0], (EasyDict, dict)):
...
def tesnorDict2numDict(tensorDict):
    numDict = {}
...
def print_opts(flats):
    ...
    for k,v in flats.items():
        # print(k,v)
        print(str(k).ljust(30)+str(v).ljust(15))
    print("\n")
